...
if __name__ == '__main__':
    ...
    ir_path = "../datasets/rgb2ir/RoadScene/trainB"
    vi_path = "../datasets/rgb2ir/RoadScene/trainA"

    ir_file_list = sorted(os.listdir(ir_path))
    vi_file_list = sorted(os.listdir(vi_path))

    ir_map_path = "../data/svs_maps/train_ir_map"
    vi_map_path = "../data/svs_maps/train_vi_map"
    ...
    # 加入 tqdm 进度条显示
    for idx, (ir_filename, vi_filename) in enumerate(
            tqdm(zip(ir_file_list, vi_file_list),
                 total=len(ir_file_list),
                 desc='Computing SVS maps')):
